...
<br/>
인터넷으로 엄청나게 검색을 해봤지만 window pc는 app data라는 폴더에서 지우면 된다는 데 인텔아이맥에서의 해결방법은 많지 않았다.         

그렇게 검색을 하다가 sqlite3 파일이 뭔지 보다보니 데이터베이스에 연결 경로를 지정하는 뭐 그런 파일인 것 같은데 이게 없어서 발생하는 문제가 아닌가 싶은 생각이 들었다.          

그래서 오류메세지에서 알려주는 폴더를 찾아가서 **filezilla 폴더를 생성**하고, **queue.sqlite3 파일을 생성**했다. 파일안에 내용은 적지 않고 그냥 파일만 만들어준뒤 다시 파일질라를 실행했더니 오류가 사라졌다!!!!!        
