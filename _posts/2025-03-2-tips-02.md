...
그래서 기존에 다른 호스팅에 연결되어있었다면 후이즈로 변경을 한뒤 네임서버 고급설정으로 들어가서 아래 링크에서 알려주는 대로 실행하면 된다.   

그리고 네임서버 변경은 적용까지 시간이 좀 걸려서 DNS Check process가 자꾸 오류가 나고있다.       
이게 입력을 잘못해서 오류인지?, 뭔가 설정이 잘못된건지 현재는 확인 불가능한 부분이지만 도메인을 주소로 들어가면 예전에 호스팅 주소로 만료안내가 나오는 걸 보니 아직 적용 전이득, 그래서 일단 github page의 커스텀 도메인은 remove 버튼을 눌러 삭제해주고, 도메인확인이 끝나면 그때 page에 다시 도메인을 등록하는 것으로!
